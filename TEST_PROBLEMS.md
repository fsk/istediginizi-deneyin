...
Öğrenme sırası: [`TEST_PROBLEMS_RANKED.md`](TEST_PROBLEMS_RANKED.md) · Base URL: `http://localhost:7070/api`
...
- [ ] **Problem:** Aynı anda birden fazla kullanıcı aynı üründen satın almaya çalışırsa, stock kontrolü yetersiz kalabilir. Uygulama stoku okuyup kontrol edip daha sonra güncelliyorsa, aynı anda gelen isteklerin çoğu stok hâlâ yeterli görünür. Stoktan fazla satış veya negatif stok oluşabilir.
...
**2) Nasıl tetiklenir? (Endpoint / istek)**

Aynı ürüne çok sayıda siparişi **aynı anda** gönder. Stok yeterli görünse bile hepsi geçebilir; sonunda stok eksi veya fazla sipariş oluşabilir.
...
```http
POST http://localhost:7070/api/orders/create-order
Content-Type: application/json

{
  "userId": "USER-UUID",
  "items": [{ "productId": "URUN-UUID", "quantity": 1 }],
...
- [ ] **Problem:** İki transaction birbirini beklerken deadlock oluşabilir. İki işlem kaynakları farklı sırada kilitlemeye çalışırsa birbirini bekleyip kilitlenir; veritabanı genelde birini geri alır.
...
Aynı anda **iki sipariş** gönder. Her siparişte **iki ürün** olsun; fark şu: bir istekte önce A sonra B, diğerinde önce B sonra A güncellensin. `createOrderSync` ürünleri `items` listesindeki sırayla işlediği için veritabanında kilit alma sırası da değişir; bu da deadlock riskini artırır.

**İstek 1** — önce `prod-A`, sonra `prod-B`:
```http
POST http://localhost:7070/api/orders/create-order
Content-Type: application/json
...
**İstek 2** — önce `prod-B`, sonra `prod-A` (sıra ters):
```http
POST http://localhost:7070/api/orders/create-order
Content-Type: application/json
...
Bu iki isteği **mümkün olduğunca aynı anda** gönder (iki terminal, Postman runner veya kısa bir script). PostgreSQL log’unda `deadlock detected` arayabilirsin; biri rollback olup hata dönebilir.

---

### c) Pessimistic Locking

- [ ] **Problem:** Satır güncellenmeden önce veritabanında kilitlenir; diğer işlemler bekler. Çakışma yerine sıra oluşur.
...
**Lab endpoint** (sipariş `PENDING` iken):
```http
POST http://localhost:7070/api/orders/{orderId}/test-concurrent-pessimistic-update?status1=CONFIRMED&status2=PROCESSING
```

**Normal kullanım:**
```http
PATCH http://localhost:7070/api/orders/{orderId}/status
Content-Type: application/json
...
- [ ] **Problem:** İki transaction aynı veriyi okur, her ikisi de günceller, son güncelleme kaybolur. İki işlem aynı değeri okuyup üzerine yazarsa birinin güncellemesi kaybolur; son yazan kazanır.
...
Lost update’i görmek için **aynı üründen**, **aynı anda** birden fazla sipariş gönder. Her sipariş stoku okuyup kendi hesabına göre yazar; ikinci işlem birincinin düşürdüğü stoğu görmeden eski değerden devam edebilir.

**Hazırlık (SQL):** Tek bir ürün seç, stoğu bilinen bir değere çek:
```sql
UPDATE products SET stock_quantity = 100 WHERE product_id = 'URUN-UUID';
```

**Örnek — İstek 1 ve İstek 2 aynı ürün, farklı miktar (paralel gönder):**
...
{
  "userId": "USER-UUID",
  "items": [{ "productId": "URUN-UUID", "quantity": 10 }],
  "shippingAddressId": "ADDRESS-UUID",
  "billingAddressId": "ADDRESS-UUID"
}
```

İstek 2 — 5 adet (aynı `productId`):
```http
POST http://localhost:7070/api/orders/create-order
Content-Type: application/json
...
{
  "userId": "USER-UUID",
  "items": [{ "productId": "URUN-UUID", "quantity": 5 }],
  "shippingAddressId": "ADDRESS-UUID",
  "billingAddressId": "ADDRESS-UUID"
}
```

Bu iki isteği **aynı anda** çalıştır (`load_test_order.py`, iki Postman sekmesi veya kısa script).

**Sonrası kontrol:**
```sql
SELECT stock_quantity FROM products WHERE product_id = 'URUN-UUID';
```

| Durum | `stock_quantity` | Anlamı |
|-------|------------------|--------|
| Doğru | **85** | 100 − 10 − 5 |
| Lost update | **95** | İkisi de 100’den okudu; son yazan 95 bıraktı, 10’luk satış “kayboldu” |
| Race (kötü) | **negatif** veya fazla sipariş | Stok koruması da yoksa |
...
Bu projede varsayılan izolasyon çoğu senaryoda bunu göstermez.
...
```http
GET http://localhost:7070/api/orders/user/USER-UUID
```
...
```http
GET http://localhost:7070/api/products/search?name=laptop
```
...
```http
GET http://localhost:7070/api/products/search?name=test
```
...
```http
PATCH http://localhost:7070/api/orders/ORDER-UUID/status
Content-Type: application/json
...
## 7. Spring Boot ve JPA Tuzakları (Projeye Özel)

### a) Stok Kontrolü Off-by-One

- [ ] **Problem:** Product.checkQuantity() stok tam sipariş miktarına eşitken yanlış sonuç döner.

**1) Senaryo**

products stock_quantity=5.

POST create-order quantity=5.

checkQuantity 5>5 false.

QuantityNotAvailableException veya yanlış red.
...
### c) @Transactional Self-Invocation (Proxy Tuzakı)

- [ ] **Problem:** Aynı sınıf içinden @Transactional çağrı transaction açmaz.

**1) Senaryo**

this.updateInTransaction() proxy olmadan.

concurrent test beklenen gibi çalışmaz.
...
### d) Geniş Transaction Scope (Uzun Transaction)

- [ ] **Problem:** createOrderSync tek transaction'da çok iş yapar.

**1) Senaryo**

user+products+order+items+saveAll.

pool ve lock süresi uzar.
...
### e) Optimistic Lock Başarısızlığında Retry Yok

- [ ] **Problem:** @Version çakışmasında 409 döner, retry yok.

**1) Senaryo**

iki order aynı product.

biri 409.

duplicate order riski.
...
### f) Open Session In View (OSIV)

- [ ] **Problem:** OSIV açıkken lazy load controller'da çalışır, N+1 gizlenir.

**1) Senaryo**

open-in-view=false yapınca LazyInitializationException.
...
### g) Hibernate JDBC Batch / saveAll Performansı

- [ ] **Problem:** saveAll batch size yoksa çok INSERT.

**1) Senaryo**

10 kalemli order.

batch kapalıysa 10+ round-trip.
...
### h) ExecutorService Yaşam Döngüsü

- [ ] **Problem:** FixedThreadPool kapatılmazsa kaynak sızıntısı.

**1) Senaryo**

ProductService/OrderService pool.
...
## 8. API, Pagination ve Doğrulama (Projeye Özel)

### a) Deep Pagination (OFFSET Maliyeti)

- [ ] **Problem:** Büyük orders tablosunda yüksek page OFFSET ile çok yavaşlar.
...
```http
GET http://localhost:7070/api/orders?page=100000&size=20&sortBy=orderDate&direction=DESC
```
...
### c) Kullanıcı Listesi Pagination'sız

- [ ] **Problem:** Tüm kullanıcılar tek seferde yüklenir.
...
```http
GET http://localhost:7070/api/users/get-all-users
```
Çok kullanıcıda yavaş / bellek baskısı.

---

### d) Order Listesi N+1 (Endpoint'e Özel)

- [ ] **Problem:** Her order için ayrı order_items sorgusu.
...
## 9. Order Lifecycle ve İş Tutarlılığı (Projeye Özel)

### a) Order Var, Payment/Shipment Yok

- [ ] **Problem:** Sipariş oluşur ödeme/kargo kaydı oluşmaz.

**1) Senaryo**

create-order sonrası payments/shipments boş.
...
### b) Bulk Order Kısmi Başarı (Partial Commit)

- [ ] **Problem:** Bulk'ta bir item fail öncekiler commit.
...
```http
POST http://localhost:7070/api/orders/bulk
Content-Type: application/json
```
...
### c) Cancel Order — Stok Geri Yükleme Yarışı

- [ ] **Problem:** İptal + eşzamanlı sipariş stok karışır.

**1) Senaryo**

cancel + create-order aynı product.
...
## 10. Konfigürasyon ve Production Hazırlığı (Projeye Özel)

### a) Hibernate ddl-auto: update

- [ ] **Problem:** Production'da şema otomatik değişir.
...
`demo-project/ecommerce/src/main/resources/application.yml` içinde ilgili ayarı değiştir, uygulamayı yeniden başlat.

---

### b) show-sql ve TRACE Logging

- [ ] **Problem:** SQL log'da performans ve PII riski.

**1) Senaryo**

show-sql true production profile.
...
## 11. Event-Driven ve Dağıtık Tutarlılik (Altyapı Hazır — Kod Eksik)

### a) Transactional Outbox Pattern

- [ ] **Problem:** DB commit ile event publish atomik değil.
...
### b) Dual Write (DB + Message Broker)

- [ ] **Problem:** DB ve MQ biri fail split brain.

**1) Senaryo**

order var payment event yok.
...
### c) Saga: Order → Payment → Shipment

- [ ] **Problem:** Çok adımda compensation yok.

**1) Senaryo**

payment ok shipment fail.
...
### d) Idempotency-Key Header (Order Create)

- [ ] **Problem:** Retry duplicate order.

**1) Senaryo**

aynı istek 2 kez 2 order.
...
## 12. PostgreSQL ve Büyük Veri Operasyonları (~50M Kayıt)

### a) Table ve Index Bloat

- [ ] **Problem:** UPDATE/DELETE sonrası tablo şişer.
...
### b) Stale Statistics (Kötü Query Plan)

- [ ] **Problem:** Planner eski stats ile seq scan.

**1) Senaryo**

bulk insert sonra analyze yok.
...
### c) Orders Tablosu Partitioning

- [ ] **Problem:** 4M+ order tek parça.
...
### d) Rapor Sorguları Timeout

- [ ] **Problem:** Aggregate raporlar timeout.

**1) Senaryo**

GET orders/report geniş tarih.

**2) Nasıl tetiklenir? (Endpoint / istek)**

```http
GET http://localhost:7070/api/orders/report?startDate=2020-01-01T00:00:00&endDate=2026-12-31T23:59:59
```

---

### e) Leading Wildcard LIKE ve Full-Text

- [ ] **Problem:** LIKE %x% index kullanamaz.
...
```http
GET http://localhost:7070/api/products/search?name=' OR '1'='1
```
...
Error mesajında: Connection to jdbc:postgresql://localhost:5432/ecommerce failed.
...
**Bu projede hazır HTTP endpoint yok** (veya sadece altyapı üzerinden).
...
products tablosuna query sayısı artar mı?

(connection pool x4).

Performance artar mı?

(throughput 2x olmalı).

Load balancing çalışır mı?

(round-robin, sticky session).

Session management problemi oluşur mu?
...
Her ikisi de commit olur → Constraint violation!
...
Thread-1: Version hala 1 mi kontrol eder → Evet!

(Yanlış!

Arada version=2 oldu).
...
Instance-2: products tablosunda product_id='abc-123', stock_quantity=50 gönderir (yanlış!

bug veya malicious).

Hangisi doğru?

Consensus mekanizması yok.
...
Instance-2: Mesajı almadı, tekrar gönderir mi?

(duplicate riski).

Instance-1: Acknowledgment almadı, tekrar gönderir mi?
...
Linearizability: Tüm node'lar aynı sırada görür mü?

(A → B veya B → A).

Sequential Consistency: Her node kendi sırasında görür mü?

(farklı node'lar farklı sıra görebilir).
...
Hepsi cache miss → 10,000 SELECT * FROM products WHERE product_id = ?
...
```http
GET http://localhost:7070/api/users/USER-UUID/test-multiple-bag-fetch
```
...
```http
POST http://localhost:7070/api/products/PRODUCT-UUID/test-concurrent-update?quantity1=10&quantity2=5
```
...
Her insert için ayrı round-trip: 1000 INSERT INTO products ...
...
saniyede key update edilir (SET product:abc-123:stock 90 EX 10).

10.

saniyede key expire olur (TTL 0'a düşer).
...
command başarısız olur (key yok: product:50).
...
command başarısız olur (key yok, INCR çalışmaz).
...
command başarısız olur (key yok: product:invalid).
...
Database query'ler yavaşlar (SELECT * FROM orders WHERE user_id = ?
...
Transaction'lar bekler (UPDATE products SET stock_quantity = ...
